...
def signup():
    ...
    if User.query.filter_by(username=username).first():
        return jsonify({
            'success': False, 
            'message': ErrorMessages.USERNAME_EXISTS
        }), HTTPStatus.BAD_REQUEST
...
def google_authorize():
    ...
    if not user_info:
        return jsonify({
            'success': False, 
            'message': ErrorMessages.GOOGLE_INFO_ERROR
        }), HTTPStatus.BAD_REQUEST
    ...
    if not user:
        ...
        if user:
            ...
            user.oauth_provider = OAuthProviders.GOOGLE
...
def auth_status():
    ...
    return jsonify({'is_authenticated': False}), HTTPStatus.OK
