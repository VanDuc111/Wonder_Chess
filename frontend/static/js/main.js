let board = null;
const STARTING_FEN =
  window.APP_CONST && window.APP_CONST.STARTING_FEN
    ? window.APP_CONST.STARTING_FEN
...
const JS_MATE_SCORE_BASE =
  window.APP_CONST &&
  window.APP_CONST.ENGINE &&
  window.APP_CONST.ENGINE.MATE_SCORE_BASE
    ? window.APP_CONST.ENGINE.MATE_SCORE_BASE
    : 1000000;
const JS_MATE_DEPTH_ADJUSTMENT =
  window.APP_CONST &&
  window.APP_CONST.ENGINE &&
  window.APP_CONST.ENGINE.MATE_DEPTH_ADJUSTMENT
    ? window.APP_CONST.ENGINE.MATE_DEPTH_ADJUSTMENT
    : 500;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadDataModalEl = document.getElementById(
    window.APP_CONST &&
      window.APP_CONST.IDS &&
      window.APP_CONST.IDS.LOAD_DATA_MODAL
      ? window.APP_CONST.IDS.LOAD_DATA_MODAL
      : "loadDataModal",
  );
  if (loadDataModalEl) {
  ...
  function initApp() {
    const displayName =
      window.USER_DATA && window.USER_DATA.isAuthenticated
        ? window.USER_DATA.displayName
        : "bạn";
    ...
    fetch(
      window.APP_CONST &&
        window.APP_CONST.API &&
        window.APP_CONST.API.CLEAR_CACHE
        ? window.APP_CONST.API.CLEAR_CACHE
        : "/api/game/clear_cache",
      { method: "POST" },
    );
  ...
  function updateEvaluationBar(score, fen) {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.updateEvaluationBar === "function"
    ) {
  ...
  function initChessboard(orientation = "white", fen = null) {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.initChessboard === "function"
    ) {
  ...
  function syncBoardAndEvalHeight() {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.syncBoardAndEvalHeight === "function"
    ) {
  ...
  function onSnapEnd() {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.onSnapEnd === "function"
    ) {
  ...
  function updateAllHighlights() {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.updateAllHighlights === "function"
    ) {
  ...
  function findKingSquare(color) {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.findKingSquare === "function"
    ) {
  ...
  function handleScoreUpdate(scoreText, fen) {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.handleScoreUpdate === "function"
    ) {
  ...
  async function handleTurnEnd(newFen) {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.handleTurnEnd === "function"
    ) {
  ...
  async function handleBotTurn() {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.handleBotTurn === "function"
    ) {
  ...
  function onDragStart(source, piece, position, orientation) {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.onDragStart === "function"
    ) {
      return window.LOGIC_GAME.onDragStart(
        source,
        piece,
        position,
        orientation,
  ...
  function updatePgnHistory() {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.updatePgnHistory === "function"
    ) {
  ...
  async function fetchDeepEvaluation(fen) {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.fetchDeepEvaluation === "function"
    ) {
  ...
  function updateButtonState() {
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.updateButtonState === "function"
    ) {
  ...
  function clearBoard() {
    ...
    if (
      window.LOGIC_GAME &&
      typeof window.LOGIC_GAME.clearBoard === "function"
    ) {
      fetch(
        window.APP_CONST &&
          window.APP_CONST.API &&
    ...
    confirmLoadBtn.addEventListener("click", async () => {
      ...
      try {
        if (activeTabId === "pgn-pane") {
          ...
          const data = await window.VISION_MANAGER.analyzeUpload(
            imageInput.files[0],
          );
          if (data.success) {
            success = true;
